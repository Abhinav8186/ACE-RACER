...
if(gameState === PLAY) {
     ...
     /* if (distance == 999) {
      ...
      if (keyDown("LEFT_ARROW") || keyDown("W") || mousePressedOver(l)) {
        car.y = car.y - 6
      }
      if (keyDown("RIGHT_ARROW") || keyDown("S") || mousePressedOver(d)) {
        car.y = car.y + 6
      }
      if (keyDown("space")) {
        horn.play()
      ...
      if (keyDown("UP")||mousePressedOver(accel)) {
        // nitro.visible = 1
        accel.addImage(go1) 
        bot2.velocityX = -8
        path.velocityX = -9 * 9
...
function c3() {
  a3 = createSprite(1500, random(50, 650))
  a3.addImage(car4);
  a3.scale = 0.4 
  a3.velocityX = -6
  a3.collide(wall)
